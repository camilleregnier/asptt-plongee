...
import com.asptt.plongee.resa.wicket.page.admin.plongee.GererPlongeeAOuvrirTwo;
...
public class InscriptionPlongeePage extends TemplatePage {
	...
	public InscriptionPlongeePage(){
		...
		this.plongeur = ResaSession.get().getAdherent(); 
		...
		try {
			 ResaSession.get().getPlongeeService().checkCertificatMedical(plongeur, null);
	...
	private void init() {
		...
		try{
			...
			plongees = ResaSession.get().getPlongeeService().rechercherPlongeePourInscriptionAdherent(plongeur);
		} catch (TechnicalException e) {
			e.printStackTrace();
			error(e.getKey());
		}
	...
	public void inscrire(AjaxRequestTarget target, IModel<Plongee> iModel) {
		...
		try {
			...
			List<Adherent> participants = ResaSession.get().getAdherentService().rechercherAdherentInscrits(plongee);
			plongee.setParticipants(participants);
			
			int response = ResaSession.get().getPlongeeService().isOkForResa(plongee, plongeur);
			...
			case 0://on inscrit l'adherent en liste d'attente sans envoi de mail : car plongee complete  
				...
				if(ResaSession.get().getPlongeeService().isOkForListeAttente(
						plongee, plongeur )){ //ResaSession.get().getAdherent()
			...
			case 3: //on inscrit un pilote ou un dp sur une plongée fermée avec envoi d'un mail aux admins
				...
				ResaSession.get().getPlongeeService().inscrireAdherent(plongee, plongeur, PlongeeMail.MAIL_INSCRIPTION_SUR_PLONGEE_FERMEE);
			...
			case 1: //on peut inscrire l'adherent à la plongee
				...
				if (null == this.parrain){
					...
					ResaSession.get().getPlongeeService().inscrireAdherent(plongee,plongeur,parrain,PlongeeMail.PAS_DE_MAIL);
				}
	...
	public class ConfirmSelectionModal extends Panel
	{
		...
		public ConfirmSelectionModal(String id, final Plongee plongee)
		{
			...
			int nbListeAttente = plongee.getParticipantsEnAttente().size()+1;
			add(new Label("infoPlongeur", message+"\n"+srmPlongeur.getString()+" "+nbListeAttente+"."));
			...
			add(new IndicatingAjaxLink("yes")
			{
				...
				public void onClick(AjaxRequestTarget target)
				{
					try{
						...
						ResaSession.get().getPlongeeService().inscrireAdherentEnListeAttente(
								plongee, plongeur != null ?  plongeur : ResaSession.get().getAdherent(), typeMail);
