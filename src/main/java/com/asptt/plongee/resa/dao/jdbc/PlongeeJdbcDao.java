...
public class PlongeeJdbcDao extends AbstractJdbcDao implements Serializable, PlongeeDao {
    ...
    /**
     * En fait on ne supprime pas l'adherent mais on init le champ
     * Date_inscription à la date du jour
     */
    @Override
    ...
    @Override
    public List<Plongee> getPlongeesWithoutFS(Date dateMin, Date dateMax)
            throws TechnicalException {
        Connection conex = null;
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
            sb.append("SELECT * FROM PLONGEE p");
            sb.append(" WHERE p.OUVERTURE_FORCEE = 1");
            sb.append(" AND p.idPLONGEES not IN (SELECT PLONGEES_idPLONGEES FROM FICHES_SECURITE)");
            sb.append(" AND date(DATE) BETWEEN ? AND ?");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
            String dateMini = sdf.format(dateMin);
            String dateMaxi = sdf.format(dateMax);
            st.setString(1, dateMini);
            st.setString(2, dateMaxi);
            log.info("request pour trouver les plongées sans FS : " + st.toString());
            ResultSet rs = st.executeQuery();
            List<Plongee> plongees = new ArrayList<Plongee>();
            while (rs.next()) {
                Plongee plongee = wrapPlongee(rs);
    ...
    @Override
    public void createFicheSecurite(FicheSecurite fs) throws TechnicalException {
        Connection conex = null;
        int idFs;
        int idPalanques;
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
            sb.append("INSERT INTO FICHES_SECURITE (`PLONGEES_idPLONGEES`, `SITE`, `METEO`, `NB_PLONGEURS`, `NB_PALANQUES`, `NB_ENCADRANTS`)");
            sb.append(" VALUES (?, ?, ?, ?, ?, ?)");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            st.setInt(1, fs.getPlongee().getId());
            st.setString(2, fs.getSite());
            st.setString(3, fs.getMeteo());
            st.setInt(4, fs.getNbPlongeurs());
            st.setInt(5, fs.getNbPalanques());
            st.setInt(6, fs.getNbEncadrants());
            if (st.executeUpdate() == 0) {
                throw new TechnicalException(
                        "La fiche de sécurité n'a pu être enregistrée");
            } else {
                // les palanques
                if (!fs.getPalanques().isEmpty()) {
                    for (Palanque palanque : fs.getPalanques()) {
                        sb = new StringBuilder();
                        sb.append("INSERT INTO PALANQUES (`NUM_PALANQUES`, `GUIDE`, `IDPLONGEUR1`, `IDPLONGEUR2`, `IDPLONGEUR3`, `IDPLONGEUR4`, `NOMGUIDE`, `NOMPLONGEUR1`, `NOMPLONGEUR2`, `NOMPLONGEUR3`, `NOMPLONGEUR4`, `PROFONDEUR_MAX_PREVUE`, `PROFONDEUR_MAX_REA`, `DUREE_TOTALE_PREVUE`, `DUREE_TOTALE_REA`, `HEURE_SORTIE`, `PALIER_3`, `PALIER_6`, `PALIER_9`, `PALIER_PROF`)");
                        sb.append(" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                        st = conex.prepareStatement(sb.toString());
                        st.setInt(1, palanque.getNumero());
                        st.setString(2, palanque.getIdGuide());
                        st.setString(3, palanque.getIdPlongeur1());
                        st.setString(4, palanque.getIdPlongeur2());
                        st.setString(5, palanque.getIdPlongeur3());
                        st.setString(6, palanque.getIdPlongeur4());
                        st.setString(7, palanque.getNomGuide());
                        st.setString(8, palanque.getNomPlongeur1());
                        st.setString(9, palanque.getNomPlongeur2());
                        st.setString(10, palanque.getNomPlongeur3());
                        st.setString(11, palanque.getNomPlongeur4());
                        st.setInt(12, palanque.getProfondeurMaxPrevue());
                        st.setInt(13, palanque.getProfondeurMaxRea());
                        st.setInt(14, palanque.getDureeTotalePrevue());
                        st.setInt(15, palanque.getDureeTotaleRea());
                        GregorianCalendar gc = new GregorianCalendar();
                        gc.setTime(fs.getPlongee().getDate());
                        gc.set(GregorianCalendar.HOUR_OF_DAY, palanque.getHeureSortie());
                        gc.set(GregorianCalendar.MINUTE, palanque.getMinuteSortie());
                        Timestamp ts = new Timestamp(gc.getTimeInMillis());
                        st.setTimestamp(16, ts);
                        st.setInt(17, palanque.getPalier3m());
                        st.setInt(18, palanque.getPalier6m());
                        st.setInt(19, palanque.getPalier9m());
                        st.setInt(20, palanque.getPalierProfond());
                        if (st.executeUpdate() == 0) {
                            //TODO Il faut supprimer la fiche de securité qui vient d'être crée.
                            throw new TechnicalException(
                                    "La palanque n'a pu être enregistrée");
                        } else {
                            //La table de relation
                            //on recupere l'id de la fiche de securite que l'on vient de créer 
                            sb = new StringBuilder();
                            sb.append("SELECT max(idFICHESECURITE) FROM FICHES_SECURITE c");
                            st = conex.prepareStatement(sb.toString());
                            ResultSet rs = st.executeQuery();
                            if (rs.next()) {
                                idFs = rs.getInt("max(idFICHESECURITE)");
                                //on recupere l'id de la fiche de securite que l'on vient de créer 
                                sb = new StringBuilder();
                                sb.append("SELECT max(idPALANQUES) FROM PALANQUES c");
                                st = conex.prepareStatement(sb.toString());
                                rs = st.executeQuery();
                                if (rs.next()) {
                                    idPalanques = rs.getInt("max(idPALANQUES)");
                                    //On crée la relation
                                    sb = new StringBuilder();
                                    sb.append("INSERT INTO REL_PALANQUES_FS (`palanques_idPALANQUES`, `fiches_securite_idFS`)");
                                    sb.append(" VALUES (?, ?)");
                                    st = conex.prepareStatement(sb.toString());
                                    st.setInt(1, idPalanques);
                                    st.setInt(2, idFs);
                                    if (st.executeUpdate() == 0) {
                                        throw new TechnicalException(
                                                "La relation FicheSecuritée palanque n'a pu être créée");
                                    }
                                }
                            }
                        }
    ...
    @Override
    public void deleteFicheSecurite(FicheSecurite fs) throws TechnicalException {
        Connection conex = null;
        int idFs = getIdFicheSecurite(fs);
        List<Integer> idsPalanques = getIdPalanques(idFs);
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
            sb.append("DELETE FROM FICHES_SECURITE WHERE IDFICHESECURITE = ? AND PLONGEES_idPLONGEES = ?");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            st.setInt(1, idFs);
            st.setInt(2, fs.getPlongee().getId());
            //requete pour la suppression de la fiche de securité
            st.executeUpdate();
            for (Integer idPalanque : idsPalanques) {
                sb = new StringBuilder();
                sb.append("DELETE FROM PALANQUES WHERE IDPALANQUES = ?");
                st = conex.prepareStatement(sb.toString());
                st.setInt(1, idPalanque);
                //Requete pour supprimer les palanques
                st.executeUpdate();
                //On supprime la relation
                sb = new StringBuilder();
                sb.append("DELETE FROM REL_PALANQUES_FS WHERE `palanques_idPALANQUES` = ? AND  `fiches_securite_idFS` = ? ");
                st = conex.prepareStatement(sb.toString());
                st.setInt(1, idPalanque);
                st.setInt(2, idFs);
                //Requete pour supprimer la relation
                st.executeUpdate();
            }
        } catch (SQLException e) {
            log.error(e.getMessage(), e);
    ...
    public int getIdFicheSecurite(FicheSecurite fs) throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
            sb.append("SELECT idFICHESECURITE FROM FICHES_SECURITE c WHERE PLONGEES_idPLONGEES = ?");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            st = conex.prepareStatement(sb.toString());
            st.setInt(1, fs.getPlongee().getId());
            ResultSet rs = st.executeQuery();
    ...
    public List<Integer> getIdPalanques(int iDfs) throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
            sb.append("SELECT PALANQUES_IDPALANQUES FROM REL_PALANQUES_FS c WHERE FICHES_SECURITE_IDFS = ?");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            st = conex.prepareStatement(sb.toString());
            st.setInt(1, iDfs);
            ResultSet rs = st.executeQuery();
