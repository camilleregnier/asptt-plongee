        ...
        try {
            ...
            if (!adherent.isEncadrent()) {
                ...
                if (ResaSession.get().getPlongeeService().rechercherListeAttente(plongee).size() > 0) {
                    ResaSession.get().getPlongeeService().deInscrireAdherent(
                            plongee,
                            ResaSession.get().getAdherent(), PlongeeMail.MAIL_PLACES_LIBRES);

                } else {
                    ResaSession.get().getPlongeeService().deInscrireAdherent(
                            plongee,
                            ResaSession.get().getAdherent(), PlongeeMail.PAS_DE_MAIL);
                }
                setResponsePage(new InscriptionConfirmationPlongeePage(plongee));
            } else {
                //C'est un encadrant : on regarde s'il en reste assez
                if (ResaSession.get().getPlongeeService().isEnoughEncadrant(plongee)) {
            ...
            add(new IndicatingAjaxLink("yes") {
                ...
                public void onClick(AjaxRequestTarget target) {
                    ...
                    try {
                        ResaSession.get().getPlongeeService().deInscrireAdherent(
                                plongee,
                                ResaSession.get().getAdherent(), PlongeeMail.MAIL_PLUS_ASSEZ_ENCADRANT);
