...
public class AdherentJdbcDao extends AbstractJdbcDao implements Serializable, AdherentDao {
    ...
    @Override
    public void deleteExterne(Adherent externe, boolean controle) throws TechnicalException {
        Connection conex = null;

        try {
            conex = getDataSource().getConnection();
            //recherche si l'externe a été inscrit à une plongée 
            StringBuilder sb = new StringBuilder();
            sb.append("SELECT * FROM INSCRIPTION_PLONGEE WHERE ADHERENT_LICENSE = ? AND DATE_ANNUL_PLONGEE is NULL");
            PreparedStatement st = conex.prepareStatement(sb.toString());
            st.setString(1, externe.getNumeroLicense());
            ResultSet rs = st.executeQuery();
            if (rs.next() && controle) {
                throw new TechnicalException("ATTENTION cet externe à été inscrit à une plongée, Voulez vous le supprimer?");
            } else {
                logger.debug("Cet externe " + externe.getNumeroLicense() + " n'a jamais été inscrit à une plongée");

                sb = new StringBuilder();
                //suppression dans la table de relation PARRAIN FIllEUL
                sb.append("DELETE FROM REL_PARRAIN_FILLEUL WHERE idFilleul = ?");
                st = conex.prepareStatement(sb.toString());
                st.setString(1, externe.getNumeroLicense());
                if (st.executeUpdate() == 0) {
                    logger.debug("Cet externe " + externe.getNumeroLicense() + " n'a jamais été parrainé");
                }
                //suppression dans la table des Liste d'Attente
                sb = new StringBuilder();
                sb.append("DELETE FROM LISTE_ATTENTE WHERE ADHERENT_LICENSE = ?");
                st = conex.prepareStatement(sb.toString());
                st.setString(1, externe.getNumeroLicense());
                if (st.executeUpdate() == 0) {
                    logger.debug("Cet externe " + externe.getNumeroLicense() + " n'a jamais été en liste d'attente");
                }
                //suppression dans la table des Inscription aux plongées
                sb = new StringBuilder();
                sb.append("DELETE FROM INSCRIPTION_PLONGEE WHERE ADHERENT_LICENSE = ?");
                st = conex.prepareStatement(sb.toString());
                st.setString(1, externe.getNumeroLicense());
                if (st.executeUpdate() == 0) {
                    logger.debug("Cet externe " + externe.getNumeroLicense() + " n'a jamais été inscrit à une plongée");
                }
                //suppression dans la table des Adherents
                sb = new StringBuilder();
                sb.append("DELETE FROM ADHERENT WHERE LICENSE = ?");
                st = conex.prepareStatement(sb.toString());
                st.setString(1, externe.getNumeroLicense());
                if (st.executeUpdate() == 0) {
                    throw new TechnicalException("L'externe n'a pu être supprimé");
                }
            }
        } catch (SQLException e) {
            log.error(e.getMessage(), e);
    ...
    public List<Adherent> getAdherentsTous() throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where (ACTIF = 1 or ACTIF = 0) and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN) order by NOM");
            ResultSet rs = st.executeQuery();
            List<Adherent> adherents = new ArrayList<Adherent>();
            while (rs.next()) {
                Adherent adherent = wrapAdherent(rs);
    ...
    public List<Adherent> getAdherentsActifs() throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where ACTIF = 1 and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN) order by NOM");
            ResultSet rs = st.executeQuery();
            List<Adherent> adherents = new ArrayList<Adherent>();
            while (rs.next()) {
                Adherent adherent = wrapAdherent(rs);
    ...
    public List<Adherent> getAdherentsInactifs() throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where ACTIF = 0 and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN) order by NOM");
            ResultSet rs = st.executeQuery();
            List<Adherent> adherents = new ArrayList<Adherent>();
            while (rs.next()) {
                Adherent adherent = wrapAdherent(rs);
    ...
    public List<Adherent> getExternes() throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where ACTIF = 2 and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN) order by NOM");
            ResultSet rs = st.executeQuery();
            List<Adherent> adherents = new ArrayList<Adherent>();
            while (rs.next()) {
                Adherent adherent = wrapAdherent(rs);
    ...
    public Adherent findById(String id) throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where LICENSE = ? and ACTIF <> 0 and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN)");
            st.setString(1, id);
            ResultSet rs = st.executeQuery();
    ...
    public Adherent authenticateAdherent(String id, String pwd) throws TechnicalException {
        ...
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = conex.prepareStatement("select * from ADHERENT where LICENSE = ?  and PASSWORD = ? and ACTIF <> 0 and (DATE_FIN is null or CURRENT_TIMESTAMP < DATE_FIN)");
            st.setString(1, id);
            st.setString(2, pwd);
            ResultSet rs = st.executeQuery();
            Adherent adherent = null;
    ...
    /**
     * Recherche les adherents inscrits à la plongée filtre sur niveau plongeur
     * si non null filtre sur encadrement si non null trie donne l'ordre de tri
     * : "nom" = nom du plongeur "date" ou null = ordre d'inscription à la
     * plongée
     */
    @Override
    ...
    @Override
    public void updateContact(ContactUrgent contact) throws TechnicalException {
        Connection conex = null;
        try {
            conex = getDataSource().getConnection();
            StringBuilder sb = new StringBuilder();
    ...
    @Override
    public void deleteContact(ContactUrgent contact, Adherent adh) throws TechnicalException {
        Connection conex = null;
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = null;
    ...
    @Override
    public void createContact(ContactUrgent contact, Adherent adh) throws TechnicalException {
        Connection conex = null;
        try {
            conex = getDataSource().getConnection();
            PreparedStatement st = null;
            ...
            if (st.executeUpdate() == 0) {
                ...
                if (rs.next()) {
                    ...
                    throw new TechnicalException(
                            "Impossible de recuperer le dernier contact créer");
                }
