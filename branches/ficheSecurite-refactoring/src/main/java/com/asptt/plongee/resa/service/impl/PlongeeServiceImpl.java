...
public class PlongeeServiceImpl implements PlongeeService, Serializable {
    ...
    /**
     * Si l'adherent n'est pas déjà inscrit dans la liste de plongées passée en
     * parametre
     */
    @Override
     ...
     * retourne 1 si ok
     *
     * 0 pour liste d'attente sans mail car plongée complète 2 plongeur est DP
     * et Pilote => ouvrir plongee 3 inscription d'un encadrant ou P4 à une
     * plongée fermée => envoi de mail à l'admin 4 inscription en liste
     * d'attente avec mail : pas assez d'encadrant 5 inscription en liste
     * d'attente avec mail : Liste d'attente déjà ouverte
     */
    @Override
    public int isOkForResa(Plongee plongee, Adherent adherent) throws ResaException, TechnicalException {
        ...
        String niveauDP = (plongee.getDp().getEncadrement() != null && !plongee.getDp().getEncadrement().equals(Encadrement.E2.name())) ? plongee.getDp().getEncadrement() : plongee.getDp().getNiveau();
    ...
    /**
     * Une plongée est ouverte s'il existe - au moins un DP et un pilote (ça
     * peut-être la même personne) reunion du 27/09/2010 - que le nombre de
     * participants (inscrit + liste d'attente) soit inferieure au nombre de
     * plongeurs max ceci pour bloquer l'inscription en cas de liste d'attente
     * sur plongée pleine
     */
    @Override
    ...
    @Override
    public void deInscrireAdherentForcee(Plongee plongee, Adherent adherent) throws ResaException, TechnicalException {
        //Appel DAO
    ...
    /**
     * Met à jour l'indicateur de suppression indic = 1 vient l'admin indic = 2
     * vient l'adherent lui même indic = 3 vient du secretariat
     */
    @Override
     ...
     * @return ResaException si Ko si nbMois =-1 => perimé si nbMois = 0 =>
     * Warning : dans le derniers mois si nbMois = 1 => Warning : mois-1 avec le
     * nombre de jour si nbMois = 2 => No soucie
     *
     */
    @Override
    ...
    @Override
    public List<Plongee> rechercherPlongeesSansFS(Date dateMin, Date dateMax) throws TechnicalException {
        List<Plongee> result = new ArrayList<Plongee>();
        for (Plongee plongee : plongeeDao.getPlongeesWithoutFS(dateMin, dateMax)) {
            if (isOuverte(plongee)) {
                result.add(plongee);
            }
        }
    ...
    @Override
    public void creerFicheSecurite(FicheSecurite fs) throws ResaException, TechnicalException {
        plongeeDao.createFicheSecurite(fs);
    }

    @Override
    public void supprimerFicheSecurite(FicheSecurite fs) throws ResaException, TechnicalException {
        plongeeDao.deleteFicheSecurite(fs);
    }
}
